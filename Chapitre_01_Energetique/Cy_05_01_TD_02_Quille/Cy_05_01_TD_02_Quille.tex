% "{'classe':('PSI'),'chapitre':'tec_3d','type':('td','normal'),'titre':'Quille pendulaire', 'source':'Concours Commun Mines Ponts 2014','comp':('C1-05','C2-08'),'corrige':True}"
%\setchapterimage{bandeau}

\chapter*{TD \arabic{cptTD} \\ 
Quille pendulaire \ifnormal $\star$ \else \fi \iftdifficile $\star\star\star$ \else \fi -- \ifprof Corrigé \else Sujet \fi}
\addcontentsline{toc}{section}{TD \arabic{cptTD} : Quille pendulaire \ifnormal $\star$ \else \fi \iftdifficile $\star\star\star$ \else \fi  -- \ifprof Corrigé \else Sujet \fi}
...
\marginnote{\xpComp{TEC}{02}\xpComp{TEC}{03}\xpComp{TEC}{04}}
%\marginnote{
...
Le modèle de calcul est donné dans les figures ci-contre.

\begin{figure}[!h]
\includegraphics[width=.7\linewidth]{fig_05_b}
...
représentées sur la figure \ref{fig:07}. 

\begin{figure}%[H]
\includegraphics[width=.7\linewidth]{fig_07}
\caption{Evolution temporelle des puissances\label{fig:07}}
\end{figure}
...
\begin{marginfigure}
\centering
\includegraphics[width=3cm]{Cy_05_01_TD_02_Quille_qr}
\end{marginfigure}
\fi
