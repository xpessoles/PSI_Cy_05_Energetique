% "{'classe':('PSI'),'chapitre':'tec_vehicule','type':('td'),'titre':'RobuROC 6 : plate-forme d’exploration tout terrain', 'source':'Concours Commun Mines Ponts 2009','comp':('C1-05','C2-08'),'corrige':False}"
...
RobuROC 6 : plate-forme d’exploration tout terrain\ifnormal $\star$ \else \fi \iftdifficile $\star\star\star$ \else \fi 
 -- \ifprof Corrigé \else Sujet \fi}
...
\marginnote{Concours Commun Mines Ponts 2009.}
\marginnote{
...
\begin{marginfigure}
\centering
\includegraphics[width=4cm]{fig_03}
%\textit{Paramétrage}
\end{marginfigure}
...
\question{Déterminer l’énergie cinétique galiléenne de l’ensemble des solides en mouvement. Le résultat sera mis sous la forme  $\dfrac{1}{2} J \dot{\varphi}^2$ où $J$ est à exprimer sous forme littérale en fonction des données du problème.}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
\question{Mettre en \oe{}uvre le théorème de l’énergie cinétique afin de déterminer l’expression du couple moteur. Vous 
...
\begin{marginfigure}
\centering
\includegraphics[width=3cm]{Cy_05_01_TD_05_Roburoc_qr}
\end{marginfigure}
\fi
...
\question{Calculer le couple moteur maximal : $C_m$ maxi. À partir du graphe de fonctionnement du moteur, conclure quand à l’aptitude de la motorisation à générer le mouvement de lacet désiré.}
\ifprof
\begin{corrige}
\end{corrige}
\else
